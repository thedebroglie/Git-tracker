 ...
 */
async function syncStudent(studentId, options = {}) {
  ...
  await invalidateCache(student.githubUsername);

  // Step 4: Fetch GitHub stats via GraphQL (will cache result)
  const rawStats = await fetchGithubStats(student.githubUsername);

  // Step 5: Anti-cheat analysis
  const { cleanedStats, flags, isFlagged } = analyze(
    rawStats,
    student.githubUsername
  );

  // Step 6: Calculate score using cleaned (post-anti-cheat) stats
  const { total, breakdown, capsApplied } = calculateScore(cleanedStats);

  // Step 7: Determine tier
  const tier = getTier(total);

  // Step 8: Calculate leaderboard rank
  const rank = await calculateRank(total, student._id);
